...
if "!NGROK_TOKEN!"=="" (
    ...
    echo         Create ngrok.env next to start.bat containing:
...
ver >nul
curl -s --max-time 5 http://localhost:!LF_PORT!/health >nul 2>&1
if errorlevel 1 (
...
echo [3/6] Opening ngrok tunnel for Langflow...
...
:wait_ngrok_lf
timeout /t 2 /nobreak >nul
for /f "delims=" %%i in ('curl -s --max-time 5 http://localhost:4040/api/tunnels 2^>nul ^| python -c "import sys,json; d=json.load(sys.stdin); t=[x[\"public_url\"] for x in d.get(\"tunnels\",[]) if x[\"public_url\"].startswith(\"https\")]; print(t[0]) if t else print(\"\")" 2^>nul') do set NGROK_LF_URL=%%i
...
ver >nul
curl -s --max-time 5 http://localhost:!ST_PORT!/_stcore/health >nul 2>&1
if errorlevel 1 (
...
:wait_ngrok_st
timeout /t 2 /nobreak >nul
for /f "delims=" %%i in ('curl -s --max-time 5 http://localhost:4040/api/tunnels 2^>nul ^| python -c "import sys,json; d=json.load(sys.stdin); t=[x[\"public_url\"] for x in d.get(\"tunnels\",[]) if x[\"public_url\"].startswith(\"https\")]; print(t[-1]) if t else print(\"\")" 2^>nul') do set NGROK_ST_URL=%%i
...
python -c "open(r'%TEMP%\pulse_update_urls.html','w').write('<html><head><meta charset=UTF-8><title>Pulse AI</title></head><body><script>var s=JSON.parse(localStorage.getItem(\"pulseai.langflow.settings.v1\")||\"{}}\");s.url=\"!LF_URL!\";s.portfolioUrl=\"!LF_PORTFOLIO_URL!\";s.key=s.key||\"\";s.streamlitUrl=\"!NGROK_ST_URL!\";localStorage.setItem(\"pulseai.langflow.settings.v1\",JSON.stringify(s));setTimeout(function(){window.location.href=\"http://localhost:5500\";},1500);</script><p style=\"font-family:sans-serif;padding:40px\">Updating settings, redirecting to Pulse AI...</p></body></html>')"

if not exist "%TEMP%\pulse_update_urls.html" (
...
timeout /t 3 /nobreak >nul
start "" "%TEMP%\pulse_update_urls.html"
